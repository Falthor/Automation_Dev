...
| `Scripts/Tests/EditMode/Presentation/GroundCoverageRendererTests.cs` | Front qui monte rangée par rangée (et rangée qui converge d'un bloc, ce qui le sépare du radial), rang pris sur le rectangle de révélation du bâtiment, front du centre vers les coins en mode radial, débordement sur les côtés mais pas sur les diagonales, avance du sol sur le bâtiment, emprise entièrement convertie à la fin de la phase du sol, seuils statiques, champ lisse insensible au poids du bruit (le grain est au shader) et plafond du grain transmis, symétrie gauche/droite du champ stocké, résolution sous-case réellement utilisée, recul du front, réupload conditionnel, cycle de vie des zones. |
...
| `Scenes/DissolveTest.unity` | Scène de validation à l'œil : une caméra, une Fonderie à sa taille de jeu (2 cases), le composant câblé. Hors Build Settings, sans aucune dépendance au reste du jeu. Ouvrir, lancer le Play, monter `Target Progress` dans l'inspecteur du composant. |
...
| Le grain du front (`noiseScale`, `noiseWeight`) | **Partagé avec le dissolve**, le sol n'a pas de réglage de grain à lui. Les deux fronts doivent être dentelés *de la même façon*, pas seulement dentelés tous les deux ; deux réglages seraient deux choses qui divergent. Appliqué **par fragment** dans le shader, jamais cuit dans la texture — voir l'écart correspondant. |
| `groundTexelsPerCell` | Résolution du champ, qui ne porte que le seuil **lisse**. Ne limite donc plus la finesse des dents — seulement la fidélité du balayage et de la frontière de débordement. À 1, cette frontière ne peut que suivre la grille. Changer la valeur réalloue les textures de zone. |
...
| **Le sol est révélé par le front du bâtiment**, sur le rectangle du bâtiment | Le seuil se lit `max(FootprintShare × rang, débordement)`. Le **rang** est l'ordre de visite : la hauteur en mode bas-vers-haut, la distance au centre en radial — le sol lit le **même `revealMode`** que le dissolve, donc les deux ne peuvent pas être réglés à contresens. En bas-vers-haut, ce rang est calculé exactement comme le `saturate((worldPos.y − _BuildBounds.y) / _BuildBounds.w)` du shader, **sur le même rectangle monde** : `DrawnSegment` transporte les bounds du renderer que le dissolve pilote. C'est cette identité, et rien d'autre, qui fait que les deux fronts sont une seule vague — deux rectangles mesurés séparément donneraient deux animations qui se contentent de démarrer ensemble. Un art qui déborde ses cases (la plupart) balaie donc plus de hauteur monde par unité d'avancement, et son sol finit d'autant plus tôt : c'est voulu, le sol doit précéder. |
| Le débordement est pris en `max`, jamais additionné | Le terme de débordement (`(sdf − coin) / groundOverflowCells`) est **négatif partout sur l'emprise** : il ne parle qu'à l'extérieur, où c'est lui — pas le bord d'un rectangle — qui arrête la conversion, dans **toutes** les directions. Le prendre en `max` plutôt qu'en somme est ce qui garde cette borne honnête : additionné, la région sous l'emprise (dont le rang vaut 0) aurait tout le budget d'avancement à dépenser en débordement, et la tache s'ouvrirait en éventail sur plusieurs cases vers le bas. |
...
| **Le débord en hauteur est porté par le pivot du sprite**, pas par un décalage de transform | Le pivot est placé au centre de l'**emprise** à l'intérieur d'une frame plus haute — 2/5 pour le Noyau, et 0,4 de la hauteur pour la Fonderie du jour où elle repassera sur une planche portrait — l'art v3 en service est carré, pivot centré, et n'a donc pas de débord à porter. Tout chemin qui positionne l'art au centre de l'emprise pose donc la base sur les bonnes cases sans rien savoir du débord, et le surplus déborde vers le haut. Un décalage de transform aurait dû être répété dans les quatre chemins, et oublié dans l'un d'eux à la première occasion. |
...
| **Le cyan est réservé par la matérialisation ; l'UI de chantier distingue donc par la texture** | Le liseré du bâtiment, le liseré du sol et la teinte de la couverture sont tous le même cyan d'accent : à l'usage le joueur a appris cette couleur comme « construction en cours », pas comme une catégorie de stock. Le panneau de chantier ne peut donc pas s'en servir pour opposer *arrivé* et *en route*. Il met les deux dans l'accent et les sépare par une **hachure à 45°** — arrivé plein, en route hachuré, manquant le fond nu de la barre. Une distinction portée par la seule teinte ne dirait rien non plus à un daltonien. USS n'a pas d'équivalent de `repeating-linear-gradient` : la hachure est tracée en **Painter2D** (`HatchFillElement`), comme `HistoryGraphElement` le fait déjà — aucune texture à importer, aucune durée de vie à gérer, et l'angle reste exact à toute taille. Chaque raie est découpée analytiquement sur le rectangle plutôt que tracée longue et laissée au rognage : une raie qui déborde irait mordre le segment voisin, c'est-à-dire exactement la frontière que l'élément existe pour montrer. |
| Le panneau de chantier répond à **deux questions à deux endroits** | Les barres décrivent le **stock** engagé (« dois-je produire ? »), la ligne de service sous le titre décrit les **robots** (« est-ce que ça avance ? »). Les confondre est ce qui faisait afficher « en route » à quatre chantiers posés ensemble alors qu'un seul était servi : leur matière était bien réservée à tous les quatre, mais un seul avait un robot qui marchait vers lui. Une réservation est une créance sur le stock, elle tient qu'un robot soit parti ou non ; « en approche » ne compte que les robots dont l'état est `MovingToSite` vers ce chantier — un robot assigné mais en route vers un coffre marche dans l'autre sens. Le mot « en route » a disparu du vocabulaire des barres pour cette raison. |
| Le compte ne détaille pas la répartition (`5 / 5`, pas `2 + 3 / 5`) | Le numérateur compte arrivé **et** réservé, donc il vaut exactement la partie remplie de la barre. Scinder le numérateur mettrait la même décomposition à deux endroits, et le compte est le moins bon des deux : la barre la donne déjà, spatialement. Pire, `3 / 8` sous une barre pleine se lit comme une contradiction et oblige le lecteur à décider lequel des deux ment. |
| L'horloge de la ligne de service est **dessinée**, pas importée | `Assets/Art/Icons` n'a pas d'horloge, et un glyphe de 12 px coûte moins cher à tracer qu'à créer, importer et maintenir à la bonne taille (`ClockGlyphElement`, Painter2D, comme la hachure). Un glyphe de police était l'autre option : écartée parce que rien ne garantit que la police du projet le porte, et qu'un glyphe absent s'affiche en tofu plutôt qu'en rien. L'icône de robot, elle, existe — et elle est prise **sur la vue des robots elle-même** plutôt que par chemin, pour que la marque du panneau soit littéralement ce que le joueur voit traverser la carte. |
| La couleur de danger du panneau se lit sur **manquant > 0**, jamais sur *livré < requis* | La distinction utile n'est pas entre arrivé et en route mais entre « le système s'en occupe » et « je dois produire » : une barre pleine de bout en bout veut dire ne rien faire, qu'elle soit pleine ou hachurée. Se déclencher sur `livré < requis` peindrait en rouge un ingrédient pendant tout le temps où les robots vont justement le chercher. Le bandeau d'état du panneau lit la même règle que les lignes — un ingrédient rouge sous un titre annonçant une livraison en cours serait le panneau qui se contredit. |

| **Le grain du sol est calculé par fragment, pas cuit dans la texture** — c'est une limite d'échantillonnage, pas un réglage | Symptôme : le front du bâtiment est dentelé, celui du sol reste **plat quoi qu'on règle**. Le bâtiment évalue son bruit *par fragment*, à 12 périodes par unité monde sur 3 octaves. Le sol lisait une texture à `groundTexelsPerCell` texels par case — 8 au maximum. Représenter 12 périodes/unité demande au moins 24 échantillons par case : cuit, le grain n'est pas seulement plus grossier, il est **sous la fréquence d'échantillonnage**, donc il ressort en ondulation lente et, à basse résolution, en bord parfaitement droit. Aucune valeur ne pouvait corriger ça. La texture ne porte donc plus que le seuil lisse, et `Custom/GroundCoverage` ajoute le grain par fragment avec la fonction et les réglages du dissolve. Corollaire : la garantie de complétude change d'énoncé — le champ ne doit plus « avoir dépassé le front » sur l'emprise mais l'avoir dépassé **d'au moins la demi-amplitude du bruit**, puisque le shader la retranche ensuite. Verrouillé par test. |
| Le bruit vit dans un `.hlsl` partagé, et le bâtiment ne l'applique pas comme le sol | `NanoNoise.hlsl` est inclus par les trois shaders (dissolve, couverture, dalle) : trois copies d'une fonction de bruit seraient trois choses qu'on peut régler séparément, et dès que deux diffèrent l'effet cesse de se lire comme un seul processus. En revanche la *façon* de l'appliquer diffère, et c'est assumé : le bâtiment tient son dégradé et son bruit dans la même unité au même moment, donc il les **mélange** (`base*(1−w) + noise*w`) ; le sol n'a jamais qu'une distance au front précalculée, donc il ne peut qu'**ajouter**. Aux poids en usage les deux sont visuellement identiques. La formule du bâtiment n'a pas été touchée — c'est celle qui est validée à l'écran. |
| La dalle applique le même grain que la couverture | Elle révèle le béton avec le même champ ; sans le même bruit, un bord lisse traînerait derrière un bord dentelé lumineux et la paire cesserait de se lire comme une seule frontière — ce qui est précisément la raison pour laquelle les deux couches partagent une texture. |
...
- `revealMode = 1` (radial) est implémenté — sol compris, les deux couches le lisent — mais n'a
  jamais été regardé à l'œil.
...
4. **Le rythme** : `assemblyRate`, en observant une vraie livraison par lots, jamais une valeur
   ...
   taille connue** : la valeur est en cases par seconde, donc juger sur une Fonderie (4 cases) et
...
3. **L'irrégularité** : `noiseWeight` puis `noiseScale` — les réglages du dissolve, partagés. Le poids déplace la frontière,
