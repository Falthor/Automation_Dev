...
> - **The Research entry arrives with the Datacenter.** The Top Bar's Research card and the Bottom Nav's Research category are both absent until the Datacenter has finished priming and powered its first cores (`ResearchPanelController.IsAvailable`, `CONTRACTS.md` §11). The panel they open takes the whole screen, in front of everything else, and is the radial research network of the design (`gdd-intro-recherche-expeditions.md` §5.4) - the Datacenter at the centre, three cores, the armament one unpowered, wheel zoom and a recentre button - not a list.
>
...
>
> - **Three sections of the imported body are Godot-era and must not be read as Unity status.** §9's exclusion list and §11's research content name types this project does not have (`Assembler`, `Laboratory`, `Research.RESEARCH_DEFS`); the Unity research tree and its effects are `CONTRACTS.md` §11. And §12's "explicitly NOT implemented" list has been overtaken twice: **notifications ship** (`NotificationSystem`, `NotificationBannerController`, `CONTRACTS.md` §15) and the **map screen ships**, opened from the slot §12 reserved for a minimap (`MAP.md` §5). Read §12 as the Godot spec's own future work, not as this project's.
>
