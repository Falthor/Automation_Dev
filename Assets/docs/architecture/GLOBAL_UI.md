...
>
> - **Keyboard, in this Unity project.** Camera and map pan on the **ZQSD cluster as printed on an AZERTY board**, which in Input System terms is `wKey`/`aKey`/`sKey`/`dKey`: `Key` names a *physical position* using the US layout as its reference, so `Key.W` is the key an AZERTY board prints "Z" on. **Anything shown to the player asks the control for its own `displayName`**, which is layout-aware; the enum name never is. Space is the pause and is deliberately ungated, so **a clicked button must not keep the focus** - UI Toolkit activates a focused `Button` on Space, which made pausing re-fire whatever was last clicked. `TopBarController` blurs it from one callback on the document root; only a `Button` is blurred, since a text field has to keep the focus a click gives it.
>
