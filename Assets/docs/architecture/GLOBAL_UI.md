...
> - **Not implemented yet, for later**: Top Bar (Power/Compute/Research cards, hover-expand, §2-3); Pause (§4); the hover-vs-selected visual distinction for the building outline (§6, we currently only have one hover outline, no separate "selected" state); contextual panels anchored to the right column instead of centered (§5); Storage aggregation's exact source list (§9 — Core's own inventory doesn't exist yet in this project); minimap beyond a bare placeholder (§12); the full style-token system (§14) beyond what individual panels already happen to match.
>
