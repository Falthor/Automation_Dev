...
- **Pull**, every tick: walks `GetInputCells()` (regardless of that neighbor's own facing); for the first neighbor exposing a `PeekPullableItem()` this building's own `CanAcceptInput` accepts, transfers it via `ConsumePulledItem`/`AddInput`. How fast a building may actually absorb what it reads is otherwise its own concern (e.g. `FoundryRuntime`'s or `StorageRuntime`'s own intake cooldown), not a side effect of transport's polling rate - **except** for one structural rule transport itself enforces on top of that, described next.
  ...
  **`ConveyorRuntime.IntakeIntervalSeconds`** (1s, i.e. 60 items/min - `TransportSystem.ConveyorItemsPerMinute`, the figure the Building menu quotes on hover): a belt accepts at most one item per interval, whichever of the three injection paths offers it (the straight-through pull, a side merge, a Splitter/Crossroad's output). The rate lives on the belt rather than in those callers for the same reason `RawOutputPullIntervalSeconds` lives here rather than on each building: a cadence three call sites have to remember to apply is a cadence one of them will forget. Deliberately equal to `RawOutputPullIntervalSeconds`, so one raw production output saturates exactly one belt and neither is silently the bottleneck.

  Three separate things, and conflating them once made a belt carry four times its rating. **Intake interval** is how often a belt takes something in. **`TransportSystem.ConveyorSpeedCellsPerSecond`** (1.5) is how fast what is already on it travels, and metering the intake does not change it - a metered belt is not a slow belt. **`ConveyorRuntime.MaxItemsPerCell`** (3, at `MinItemSpacing` apart) is how tightly items may pack once the line ahead is blocked: a jam buffer, never a rate. In free flow the intake spaces items 1.5 cells apart by itself and a belt normally carries one item or none; only a blockage brings them up against each other.
