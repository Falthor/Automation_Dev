...
**Le test qui garde l'absence de sauvegarde n'assure pas « rien n'est sauvegardé »** — c'est
impossible à écrire directement — mais que ce que la découverte capture est identique à l'octet, avec
ou sans observateur. C'est la forme qu'il faut chercher quand ce qu'on veut garantir est une absence.

**Le renommage a coûté cinq lignes dans un seul fichier**, parce que tout le reste du projet passait
déjà par `IsDiscovered()` : c'est exactement le bénéfice qu'un accesseur nommé achète, et il ne se voit
que le jour où on renomme ce qu'il cache.
...
un ordre d'appel heureux.

**Les deux champs ne changent pas à la même horloge, et c'est ce qui a dicté le découpage.** La
...
**Les deux frontières sont coupées par le même grain en espace monde**, et les deux alternatives ont
été essayées : deux bruits indépendants donnent deux ondulations sans rapport, et un grain en espace
écran fait ramper le bord de l'observation en suivant le robot.
