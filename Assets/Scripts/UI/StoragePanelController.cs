...
namespace Game.UI
{
    ...
    public sealed class StoragePanelController : MonoBehaviour
    {
        ...
        VisualElement BuildCard(string itemId, int amount, int slotIndex)
        {
            ...
            // The display name, never the id: copper_plate is what the project calls it and
            // "Plaque de cuivre" is what the player does. An item missing from the database falls
            // back to its id rather than to an empty tooltip, because a name that says nothing is
            // worse than one that says something odd - and it points straight at the missing
            // definition. UI Toolkit's own tooltip, not a custom label: it appears on hover without
            // a click to open it or a click elsewhere to dismiss it.
            ItemDefinition item = gameRuntime.Items != null ? gameRuntime.Items.Get(itemId) : null;
            card.tooltip = item != null ? item.DisplayName : itemId;
            ...
            // Right-click offers to throw the stack away, gated on slotIndex: there is nothing to
            // discard from an aggregate row, and a menu that opens on nothing teaches the player the
